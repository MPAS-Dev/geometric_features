...
def feature_already_exists(existing_features, adding_feature):  # {{{
    ...
    for feature in existing_features['features']:
        try:
            ...
            print "A feature in the existing features file does not have a " \
                "name property. Exiting..."
            quit(1)

        if test_name == feature_name:
            print " A feature already exists with the name '%s', either use " \
                "that one, or change the name before adding it." \
                % (feature_name)
            return True
