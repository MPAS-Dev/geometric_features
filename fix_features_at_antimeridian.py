...
def splitGeometryCrossingAntimeridian(geometry):
    def toPolar(lon, lat):
        ...
        # testing wedge
        phi = numpy.sign(phi) * \
            numpy.where(numpy.abs(phi) > numpy.pi - 1.5*epsilon,
                        numpy.pi - 1.5*epsilon, numpy.abs(phi))
    ...
    def fromPolar(x, y):
        ...
        phi = numpy.sign(phi) * \
            numpy.where(numpy.abs(phi) > numpy.pi - 2*epsilon,
                        numpy.pi, numpy.abs(phi))
...
parser.add_argument("-f", "--features_file", dest="features_file",
                    help="File containing features to split at antimeridian",
