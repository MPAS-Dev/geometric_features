...
"""
This script creates 6 major ocean basins, 5 regions used for computing the
meridional overturning circulation (MOC) and 4 transects defining the southern
boundaries of each MOC region.  No arguments are required. The optional --plot
flag can be used to produce plots of each basin or region.
...
def extract_southern_boundary(inFileName, outFileName, minLat):  # {{{
    ...
    for feature in inFeatures['features']:
        name = feature['properties']['name']
        feature['properties']['name'] = '%s southern boundary' % name
        if feature_already_exists(features, feature):
            print "Warning: feature %s already in features.geojson.  " \
                  "Skipping..." % feature['properties']['name']
            continue
        geom = feature['geometry']
        boundaryShapes = []
        if geom['type'] == 'Polygon':
            featureShape = shapely.geometry.shape(geom)
            boundaryShapes.append(find_southern_boundary(featureShape.exterior,
                                                         minLat))
        elif geom['type'] == 'MultiPolygon':
            featureShape = shapely.geometry.shape(geom)
            for polygon in featureShape:
                boundaryShapes.append(find_southern_boundary(polygon.exterior,
                                                             minLat))
        feature['geometry'] = shapely.geometry.mapping(
            shapely.ops.cascaded_union(boundaryShapes))
        features['features'].append(feature)

    write_all_features(features, outFileName, indent=4)  # }}}


def find_southern_boundary(boundary, minLat):  # {{{

    # drop the last coordinate because it's cyclic
    coords = numpy.array(boundary.coords[:])[0:-1, :]
    indices = numpy.nonzero(coords[:, 1] == minLat)[0]
    # find the western-most point at minLat
    minIndex = indices[numpy.argmin(coords[indices, 0])]

    # find the eastern-most point at minLat
    maxIndex = indices[numpy.argmax(coords[indices, 0])]
    ...
    pathIndices1 = numpy.arange(minIndex, maxIndex+1)
    pathIndices2 = numpy.mod(numpy.arange(maxIndex,
                                          minIndex+1+coords.shape[0]),
                             coords.shape[0])

    meanLat1 = numpy.mean(coords[pathIndices1, 1])
    meanLat2 = numpy.mean(coords[pathIndices2, 1])
    if meanLat1 < meanLat2:
        return shapely.geometry.LineString(coords[pathIndices1, :])
    else:
        return shapely.geometry.LineString(coords[pathIndices2, :])  # }}}
...
def build_MOC_basins():  # {{{
    ...
    MOCSouthernBoundaryFileName = 'MOCSouthernBoundaries.geojson'
    ...
    tempSubdividedSouthernBoundaryFileName \
        = 'temp_subdivided_southern_boundary.geojson'
    ...
    for basinName in MOCSubBasins:
        ...
        # extract the southern boundary
        extract_southern_boundary(tempMOCLargePolygonsFileName,
                                  tempSouthernBoundaryFileName,
                                  minLat=MOCMinLat[basinName])

        # subdivide southern boundaries into 0.1 degree segments
        spcall(['./subdivide_transects.py', '-f', tempSouthernBoundaryFileName,
                '-o', tempSubdividedSouthernBoundaryFileName,
                '-m', '0.1'])

        spcall(['./merge_features.py', '-f',
                tempSubdividedSouthernBoundaryFileName,
