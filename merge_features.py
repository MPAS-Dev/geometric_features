...
parser.add_argument("-f", "--feature_file", dest="feature_file",
                    help="Single feature file to append to features.geojson",
                    metavar="FILE")
parser.add_argument("-d", "--features_directory", dest="features_dir",
                    help="Directory containing multiple feature files, each "
                    "will be appended to features.geojson", metavar="PATH")
parser.add_argument("-t", "--tags", dest="tags",
                    help="Semicolon separated list of tags to match features "
                    "against.", metavar='"TAG1;TAG2;TAG3"')
...
if not args.feature_file and not args.features_dir:
    parser.error('Either a feature file (-f) or a feature directory (-d) is '
                 'required.')

if args.features_dir:
    if not os.path.exists(args.features_dir):
        parser.error('The path {} does not exist.'.format(args.features_dir))
